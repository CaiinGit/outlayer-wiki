...
test('legacy trees retain their nodes and gain two independent empty paths',()=>{const {prepareAffinity}=require('../skills/graph.js');const original={name:'Test',...graph};const updated=prepareAffinity(original);assert.deepEqual(updated.nodes,graph.nodes);assert.equal(updated.paths.length,2);updated.paths[0].nodes.push({id:'new'});assert.equal(updated.paths[1].nodes.length,0);assert.equal(original.paths,undefined);});
